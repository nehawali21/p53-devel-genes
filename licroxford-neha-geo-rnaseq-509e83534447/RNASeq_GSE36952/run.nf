	...
	main:
		...
		Channel.fromPath("${params.inputTable}") \
			...
			| multiMap { it ->
				...
				fastqs: [it["Read.Group"], ["${params.dataDir}/" + it["R1.File"], "${params.dataDir}/" + it["R2.File"]]]
				} \
			| set { metadata }
		...
		metadata.fastqs \
			| map { id, paths -> tuple(id, paths, [twoColour: params.minQuality, minOverlap: params.minOverlap, fwAdapter: params.fwAdapter, rvAdapter: params.rvAdapter, clipR1: params.trimLength.toInteger(), clipR2: params.trimLength.toInteger()])} \
			| trimReads	
